...
export default {
  ...
  methods: {
    ...
    onSort(column) {
      ...
      const sortedArray = [...this.market];
      sortedArray.sort((a, b) => {
        const res = a[column] < b[column] ? -1 : a[column] > b[column] ? 1 : 0;
        return this.direction === 'asc' ? res : -res;
      });
      this.market = sortedArray;
    },
  },
};
</script>
...
<template>
  <Layout>
    <PageHeader title="My Wallet" pageTitle="Crypto" />
    <BRow>
      <BCol xxl="9">
        <BCard no-body>
          <BCardHeader>
            <div class="d-flex align-items-center">
              ...
              <div class="toolbar d-flex align-items-start justify-content-center flex-wrap gap-2">
                <BButton type="button" variant="soft-primary" size="sm" class=" shadow-none timeline-btn" id="one_month">
                  1M
                </BButton>
                <BButton type="button" variant="soft-primary" size="sm" class="timeline-btn shadow-none " id="six_months">
                  6M
                </BButton>
                <BButton type="button" variant="soft-primary" size="sm" class="timeline-btn shadow-none " id="one_year">
                  1Y
                </BButton>
                <BButton type="button" variant="soft-primary" size="sm" class="timeline-btn shadow-none  active" id="all">
                  ALL
                </BButton>
              </div>
            </div>
          ...
          <template v-for="(item, index) of cryptoSlider" :key="index">
            <swiper-slide>
              <BCard no-body>
                <BCardBody>
                  <div class="float-end">
                    <BDropdown variant="link" toggle-class="fs-18 arrow-none shadow-none" menu-class="dropdown-menu-end"
                  ...
                  <div class="d-flex align-items-center">
                    <img :src="item.img" class="bg-light rounded-circle p-1 avatar-xs img-fluid shadowarket Stat" alt />
        ...
        <BCard no-body id="marketList">
          <BCardHeader class="border-bottom-dashed d-flex align-items-center">
            <BCardTitle class="mb-0 flex-grow-1">Market Status</BCardTitle>
            <div class="flex-shrink-0">
              <b-btn-group class="btn-group shadow">
                <BButton type="button" variant="primary" size="sm" class="shadow-none">
                  Today
                </BButton>
                <BButton type="button" variant="outline-primary" size="sm" class="shadow-none">
                  Overall
                </BButton>
              </b-btn-group>
            </div>
            ...
            <div class="table-responsive table-card">
              <table class="table align-middle table-nowrap" id="customerTable">
                <thead class="table-light text-muted">
                  <tr>
                    <th class="sort" data-sort="currency_name" scope="col" @click="onSort('currency_name')">
                      Name
                    </th>
                    <th class="sort" data-sort="quantity_value" scope="col" @click="onSort('quantity')">
                      Quantity
                    </th>
                    <th class="sort" data-sort="avg_price" scope="col" @click="onSort('price')">
                      Avg. Price
                    </th>
                    <th class="sort" data-sort="current_value" scope="col" @click="onSort('value')">
                      Current Value
                    </th>
                    <th class="sort" data-sort="returns" scope="col" @click="onSort('returns')">
                      Returns
                    </th>
                    <th class="sort" data-sort="returns_per" scope="col" @click="onSort('volume')">
                      Returns %
                    </th>
                  </tr>
                </thead>
        ...
        <BCard no-body>
          ...
          <BCardBody>
            ...
            <div class="d-flex mb-3">
              ...
              <div>
                <h6 class="text-success btn mb-0 shadow-none">+ $7,365.80</h6>
              </div>
            </div>
            <div>
              <Link href="/crypto/transactions" class="btn btn-soft-info w-100">View All Transactions
