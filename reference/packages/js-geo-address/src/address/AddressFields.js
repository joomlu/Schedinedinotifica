// Source copied from reference/libreria/address/AddressFields.js
export default (function (root, factory) {
  ...
  AddressFields.prototype.init = function(){
    ...
    ['change','input'].forEach(function(evt){
      Object.keys(self.ids).forEach(function(k){
        var el = self.ids[k]; if (el && el.tagName) el.addEventListener(evt, function(){ self.renderInline(); self.notify(); });
      });
    });
    self.renderInline();
  };
