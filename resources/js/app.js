/*
App: Tanggo - Pannello di Amministrazione
Maintainer: Prospero Soft
Note: File JS principale dell'applicazione
*/
...
(function () {
    ...
    function pluginData() {
        ...
        Array.from(flatpickrExamples).forEach(function (item) {
            if (item.getAttribute("data-provider") == "flatpickr") {
                ...
                // Force Italian locale globally (flatpickr Italian file is loaded in vendor-scripts)
                dateData.locale = 'it';
                // Evita il dropdown del mese (che appare come select bianco): usa header statico con frecce
                dateData.monthSelectorType = 'static';
                ...
                // Non usare altInput per mantenere l'input base di flatpickr senza secondo campo
                // Ignoriamo data-altFormat così da evitare UI alternative che possono sovrapporsi alle label
    ...
    // Inizializza vincoli tra coppie di date flatpickr marcate con data-date-pair e data-date-pair-role
    // Regola: end > start (strettamente). Applica minDate su end e maxDate su start in modo simmetrico.
    function initDatePairConstraints() {
        try {
            var nodes = document.querySelectorAll('input[data-date-pair][data-provider="flatpickr"]:not([data-range-date])');
            if (!nodes || !nodes.length) return;
            var groups = {};
            Array.from(nodes).forEach(function (el) {
                var g = el.getAttribute('data-date-pair');
                var role = el.getAttribute('data-date-pair-role');
                if (!g || !role) return;
                if (!groups[g]) groups[g] = {};
                groups[g][role] = el;
            });
            Object.keys(groups).forEach(function (g) {
                var startEl = groups[g].start;
                var endEl = groups[g].end;
                if (!startEl || !endEl) return;
                var sp = startEl._flatpickr;
                var ep = endEl._flatpickr;
                if (!sp || !ep) return;
                function fromStart() {
                    var sd = sp.selectedDates && sp.selectedDates[0];
                    if (sd) {
                        var min = new Date(sd.getTime());
                        min.setDate(min.getDate() + 1); // end strettamente dopo start
                        ep.set('minDate', min);
                        var ed = ep.selectedDates && ep.selectedDates[0];
                        if (ed && ed.getTime() <= sd.getTime()) ep.clear();
                    } else {
                        ep.set('minDate', null);
                    }
                }
                function fromEnd() {
                    ...
                    if (ed) {
                        ...
                        max.setDate(max.getDate() - 1); // start strettamente prima di end
                        sp.set('maxDate', max);
                        var sd = sp.selectedDates && sp.selectedDates[0];
                        if (sd && sd.getTime() >= ed.getTime()) sp.clear();
                    } else {
                        sp.set('maxDate', null);
                    }
                }
                sp.config.onChange.push(fromStart);
                ep.config.onChange.push(fromEnd);
                // Prima applicazione
                fromStart();
                fromEnd();
            });
        } catch (e) {
            console.error('initDatePairConstraints error', e);
        }
    }
