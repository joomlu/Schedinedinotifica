...
class GeoItaliaUI {
    constructor(root) {
        ...
        this.resolveSeq = 0;
        this.suppressCapChangeResolve = false;
        this.state = {
            nazione: null,
            regione: null,
            provincia: null,
    ...
    isItaliaNation(data = null) {
        const source = data || this.state.nazione || {};
        const iso = (source.codice_iso2 || '').toUpperCase();
        const text = `${source.text || source.nome || source.label || ''}`.toLowerCase();
        return !!source.is_italia || iso === 'IT' || text.includes('italia');
    }
    ...
    initNazioni() {
        ...
        $(el).on('select2:select', (e) => {
            ...
            if (isItaly) {
                ...
                // In estero mode keep the selected nation; only hide IT-only guided fields.
    ...
    initCap() {
        ...
        $(el).on('select2:select', (e) => {
            ...
            if (data && data.id) {
                ...
                // Evita doppio resolve: select2:select già avvia la risoluzione.
                this.suppressCapChangeResolve = true;
        ...
        $(el).on('change', () => {
            const val = el.value;
            if (this.suppressCapChangeResolve) {
                this.suppressCapChangeResolve = false;
                this.emitChange();
                return;
            }
            ...
            const isNumericCap = /^[0-9]{3,10}$/.test(normalizedVal);
            if (normalizedVal && normalizedVal.length >= 3 && isNumericCap) {
                this.clearResolvedHierarchy(true);
                this.state.cap = { id: normalizedVal, text: normalizedVal };
                this.resolve({ cap: normalizedVal });
            }
            this.emitChange();
        });
        ...
        if (allowManualEntry) {
            $cap.on('select2:close', () => {
                ...
                const isNumericCap = /^[0-9]{3,10}$/.test(term);
                if (term && isNumericCap && term !== currentVal) {
                    // La close manuale imposta l'opzione e chiama resolve una sola volta.
                    this.suppressCapChangeResolve = true;
    ...
    enterEsteroMode(opts = {}) {
        ...
        this.updateModeUI();
        if (this.nodes.nazione && typeof $ !== 'undefined' && this.isItaliaNation()) {
            const placeholder = this.nodes.nazione.dataset.placeholder || this.nodes.nazione.getAttribute('placeholder') || '';
            const mask = () => {
                // Guard runtime: se nel frattempo è stata selezionata una nazione estera,
                // non sovrascrivere il render con il placeholder.
                if (!this.isItaliaNation()) {
                    return;
                }
    ...
    updateModeUI() {
        ...
        if (this.manualMode && this.nodes.nazione && typeof $ !== 'undefined' && this.isItaliaNation()) {
            const placeholder = this.nodes.nazione.dataset.placeholder || this.nodes.nazione.getAttribute('placeholder') || '';
            const mask = () => {
                // Guard runtime: se la nazione corrente non è Italia, non mascherare il valore.
                if (!this.isItaliaNation()) {
                    return;
                }
                const inst = $(this.nodes.nazione).data('select2');
    ...
    applyInitial() {
        ...
        if (initial.cap) {
            ensureOption(this.nodes.cap, { id: initial.cap, text: initial.cap }, false);
        }
    ...
    hydrateFromResolve(data) {
        ...
        if (!this.manualMode && caps && Array.isArray(caps) && caps.length) {
            ...
            if (hasCurrentCap) {
                ...
                ensureOption(this.nodes.cap, { id: currentCapValue, text: currentCapValue }, false);
