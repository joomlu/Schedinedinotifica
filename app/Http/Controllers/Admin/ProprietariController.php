...
class ProprietariController extends Controller
{
    ...
    {
        $admin = $request->user();
        $proprietari = Proprietario::with('admin')
            ->withCount('strutture')
    ...
    public function create(Request $request)
    {
        return view('superadmin.proprietari.form', [
            ...
            'strutturaRoutePrefix' => 'admin.strutture',
            'admin' => $request->user(),
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $proprietario = Proprietario::create($payload);

        return redirect()
            ->route('admin.proprietari.edit', ['id' => $proprietario->id, 'tab' => $data['active_tab'] ?? 'personale'])
    ...
    {
        $admin = $request->user();
        $proprietario = $this->findOwnedProprietario($request, $id, ['admin', 'servizi', 'strutture']);
        ...
        return view('superadmin.proprietari.form', [
            ...
            'struttureDisponibili' => Struttura::query()
                ...
                ->where(function ($query) use ($admin) {
                    $query->whereNull('proprietario_id')
                        ->orWhereHas('proprietario', fn ($subQuery) => $subQuery->where('admin_id', $admin->id));
                })
    ...
    {
        $proprietario = $this->findOwnedProprietario($request, $id, 'strutture');
        $data = $this->validateProprietario($request);
    ...
    public function createProformaPage(Request $request, int $id)
    {
        $proprietario = $this->findOwnedProprietario($request, $id, ['admin', 'servizi', 'strutture']);
    ...
    {
        $proprietario = $this->findOwnedProprietario($request, $id, ['admin', 'strutture']);
        ...
        return redirect()
            ->route('admin.proprietari.proforme.show', ['id' => $proprietario->id, 'fatturazione' => $fattura->id])
            ->with('success', 'Proforma creata correttamente.');
    }

    public function showProforma(Request $request, int $id, int $fatturazione)
    {
        $proprietario = $this->findOwnedProprietario($request, $id);
    ...
    public function editProforma(Request $request, int $id, int $fatturazione)
    {
        ...
        if ($proforma->stato !== 'proforma') {
            return redirect()
                ->route('admin.proprietari.proforme.show', ['id' => $proprietario->id, 'fatturazione' => $proforma->id])
    ...
    public function closeProforma(Request $request, int $id, int $fatturazione)
    {
        ...
        return redirect()->route('admin.proprietari.proforme.show', ['id' => $proprietario->id, 'fatturazione' => $proforma->id])->with('success', 'Proforma chiusa.');
    }

    public function markProformaFatturata(Request $request, int $id, int $fatturazione)
    {
        $proprietario = $this->findOwnedProprietario($request, $id);
        $proforma = ProprietarioFatturazione::where('proprietario_id', $proprietario->id)->findOrFail($fatturazione);
        ...
        return redirect()->route('admin.proprietari.proforme.show', ['id' => $proprietario->id, 'fatturazione' => $proforma->id])->with('success', 'Proforma segnata come fatturata.');
    }
    ...
    public function disable(Request $request, int $id)
    {
        ...
        return redirect()->route('admin.proprietari.index')->with('success', 'Proprietario disabilitato.');
    }
    ...
    public function assegnaStruttura(Request $request, int $id)
    {
        ...
        if ($struttura->proprietario && $struttura->proprietario->admin_id !== $request->user()->id) {
            abort(403);
        }
        ...
        return redirect()
            ->route('admin.proprietari.edit', ['id' => $proprietario->id, 'tab' => 'strutture'])
            ->with('success', 'Struttura assegnata al proprietario.');
    }

    private function findOwnedProprietario(Request $request, int $id, $with = [])
    {
        return Proprietario::with($with)
