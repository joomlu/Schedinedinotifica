...
class StruttureController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        return view('admin.strutture.form', $this->buildFormViewData(
            new Struttura(),
            $proprietari,
            'create'
        ));
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($data['proprietario_id']) && !in_array((int) $data['proprietario_id'], array_map('intval', $proprietariIds), true)) {
            abort(403);
        }
    ...
    private function loadProformeStorico(Struttura $struttura)
    {
        if (!$struttura->exists) {
            return collect();
        }

        return ProprietarioFatturazione::query()
            ->with([
                'proprietario',
                'righe' => function ($query) use ($struttura) {
                    $query->where('struttura_id', $struttura->id)
                        ->with(['servizio', 'struttura']);
                },
            ])
    ...
    private function buildZoneOptions(Struttura $struttura, ?int $geoComuneId, string $tipo): array
    {
        ...
        $catalogo = StrutturaZona::query()
            ->where('tipo', $tipo)
            ->where('attiva', true)
            ->when($struttura->exists, function ($query) use ($struttura) {
                $query->where(function ($sub) use ($struttura) {
                    $sub->whereNull('struttura_id')
                        ->orWhere('struttura_id', $struttura->id);
                });
            }, function ($query) {
                $query->whereNull('struttura_id');
            })
