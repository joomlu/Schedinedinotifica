...
// Sezione DEMO e LIBRERIA: solo in ambiente local o con APP_DEBUG abilitato
if (app()->environment('local') || config('app.debug')) {
    // Pagina orfana di test: Nac_Reg_Prov_Citt (demo Select2 e relazione geografica)
    Route::get('/Nac_Reg_Prov_Citt', function () {
        return view('dev.nac_reg_prov_citt');
    })->name('dev.geo-demo');

    // Pagina orfana di test: Ind_Tipo_Via_Num_Int (demo Indirizzo)
    Route::get('/Ind_Tipo_Via_Num_Int', function () {
        return view('dev.ind_tipo_via_num_int');
    })->name('dev.address-demo');

    // Nuove pagine modello in italiano
    Route::get('/Relazione-Geografica', function () {
        return view('dev.relazione_geografica');
    })->name('dev.relazione-geografica');

    Route::get('/Indirizzo', function () {
        return view('dev.indirizzo');
    })->name('dev.indirizzo');

    // Servizio file libreria UMD (solo dev): espone i JS dalla cartella reference/libreria
    Route::get('/libreria/GeoSelect.js', function () {
        $path = base_path('reference/libreria/geo/GeoSelect.js');
        if (! file_exists($path)) {
            abort(404);
        }

        return response()->file($path, ['Content-Type' => 'application/javascript']);
    })->name('libreria.geoselect');

    Route::get('/libreria/AddressFields.js', function () {
        $path = base_path('reference/libreria/address/AddressFields.js');
        if (! file_exists($path)) {
            abort(404);
        }

        return response()->file($path, ['Content-Type' => 'application/javascript']);
    })->name('libreria.addressfields');
}
